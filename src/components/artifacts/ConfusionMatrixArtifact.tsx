...
export function ConfusionMatrixArtifact({ artifact }: { artifact: Artifact }) {
  ...
  return (
    <div className="p-3 space-y-3">
      <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[9px] font-mono uppercase tracking-wider ${
        ...
        {verified
          ? <><ShieldCheck className="w-3 h-3" /> Verified · real holdout</>
          : <><AlertTriangle className="w-3 h-3" /> Estimated · not from a trained model</>}
      </div>
      {/* Metrics row */}
