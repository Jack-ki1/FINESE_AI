  ...
  if (collapsed) {
    return (
      <div className="w-10 shrink-0 border-l bg-card flex flex-col items-center py-3 gap-2">
        ...
        <span className="text-[10px] font-mono bg-[hsl(var(--verified))]/15 text-[hsl(var(--verified))] px-1.5 py-0.5 rounded">{evidence.filter(e=>e.verified).length}✓</span>
      </div>
    );
  }

  return (
    <div className="w-[300px] shrink-0 border-l bg-[hsl(var(--surface))] flex flex-col overflow-hidden">
      <div className="h-12 flex items-center justify-between px-3 border-b shrink-0">
        <div className="flex items-center gap-2">
          <FlaskConical className="w-4 h-4 text-[hsl(var(--verified))]" />
          <span className="text-xs font-semibold tracking-wide uppercase">Evidence</span>
          <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-[hsl(var(--verified))]/15 text-[hsl(var(--verified))] border border-[hsl(var(--verified))]/20">{evidence.filter(e=>e.verified).length} verified</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {evidence.length === 0 ? (
          ...
          <div key={i} className={`rounded-lg border p-2.5 space-y-1 ${e.verified ? 'bg-white dark:bg-card border-[hsl(var(--verified))]/30' : e.offline ? 'bg-card border-[hsl(var(--estimated))]/30 border-dashed' : 'bg-card border-border'}`}>
            <div className="flex items-center gap-2">
              <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded-full border font-semibold uppercase tracking-wider ${e.verified ? 'bg-[hsl(var(--verified))]/15 text-[hsl(var(--verified))] border-[hsl(var(--verified))]/30' : e.offline ? 'bg-[hsl(var(--estimated))]/10 text-[hsl(var(--estimated))] border-[hsl(var(--estimated))]/30 border-dashed' : 'bg-[hsl(var(--estimated))]/10 text-[hsl(var(--estimated))] border-[hsl(var(--estimated))]/30 border-dashed'}`}>
      ...
      <div className="p-3 border-t bg-card">
        <p className="text-[11px] leading-relaxed text-muted-foreground">
          <span className="font-mono text-[hsl(var(--verified))] font-semibold">Teal = server-verified</span> · <span className="font-mono text-[hsl(var(--estimated))]">ochre dashed = estimated</span>. Numbers in mono are from compute, not the model.
        </p>
      </div>
    </div>
  );
