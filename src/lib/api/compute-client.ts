...
export async function callComputeTool(tool: string, args: any, file_hash: string) {
  // Server path — skipped entirely in local mode (no backend exists).
  if (!isLocalMode()) {
    try {
  ...
  // Local computation — in local mode this is the primary path, otherwise
  // the offline-preview fallback. Explicitly marked, never masquerades as
  // Verified.
  const raw = localStorage.getItem(`finese-dataset-${file_hash}`);
  if (!raw) throw new Error("Dataset not available locally — upload it again.");
